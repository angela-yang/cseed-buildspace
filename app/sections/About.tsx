...
const resources = [
  {
    ...
    bg: "rgba(57,123,255,0.09)",
    border: "rgba(57,123,255,0.2)",
  ...
  },
  {
    ...
    color: "rgb(136,0,185)",
    bg: "rgba(136,0,185,0.09)",
    border: "rgba(136,0,185,0.2)",
...
const ResourceCard = ({ resource, idx }: { resource: typeof resources[0]; idx: number }) => {
  return (
    <motion.div
      initial={{ opacity: 0, x: resource.fromX, rotate: resource.fromX > 0 ? 3 : -3 }}
      whileInView={{ opacity: 1, x: 0, rotate: 0 }}
      viewport={{ once: true, margin: "-30px" }}
      transition={{ duration: 0.75, ease: [0.25, 0.46, 0.45, 0.94], delay: idx * 0.08 }}
      animate={{
        y: [0, -8, 0],
      }}
      style={{ marginLeft: resource.offsetX }}
      className="w-[88%] md:w-[75%]"
    >
      <div
        className="relative rounded-3xl overflow-hidden flex flex-row items-stretch"
        style={{
          background: resource.bg,
          border: `2px solid ${resource.border}`,
          boxShadow: `0 8px 40px ${resource.color}22, 0 2px 12px rgba(0,0,0,0.07)`,
          minHeight: "clamp(130px, 18vw, 200px)",
        }}
      >
        <div className="w-2 shrink-0" style={{ background: resource.color }} />

        <div className="flex-1 px-6 md:px-8 py-6 md:py-7 flex flex-col justify-center gap-2">
          <h3
            className="font-black leading-tight"
            style={{ color: resource.color, fontSize: "clamp(1.5rem, 3.0vw, 1.7rem)" }}
          >
            {resource.title}
          </h3>
          <p className="text-gray-600 ibm-plex-sans leading-relaxed" style={{ fontSize: "clamp(0.90rem, 1.5vw, 1.3rem)" }}>
            {resource.desc}
          </p>
        </div>

        <div
          className="relative shrink-0 flex items-end justify-center overflow-visible"
          style={{ width: "clamp(100px, 18vw, 180px)" }}
        >
          <motion.img
            src={resource.cutout}
            alt=""
            ...
            style={{
              ...
              objectFit: "contain",
              filter: `drop-shadow(0 8px 20px ${resource.color}55)`,
            }}
          />
        </div>
      </div>
...
const FloatingResourceCard = ({ resource, idx }: { resource: typeof resources[0]; idx: number }) => {
  return (
    <motion.div
      animate={{ y: [0, -10, 0] }}
      transition={{
        ...
        repeat: Infinity,
        ease: "easeInOut",
        delay: resource.floatDelay,
      }}
      className="flex"
      style={{ justifyContent: resource.fromX > 0 ? "flex-end" : "flex-start" }}
    >
      <ResourceCard resource={resource} idx={idx} />
    </motion.div>
  );
};
...
export default function About() {
  ...
  return (
    <main id="about" className="section">
      ...
      {/* Resource Cards */}
      <section className="relative px-[5vw] md:px-[8vw] mb-40 mt-4">
        <ScrollReveal>
          <h2 className="text-4xl md:text-5xl font-bold text-center mb-4 text-[rgb(57,123,255)]">
            What You Get
          </h2>
          <p className="text-center text-2xl text-gray-500 ibm-plex-sans mb-16 max-w-4xl mx-auto">
        ...
        <div className="flex flex-col gap-6 max-w-4xl mx-auto">
          {resources.map((resource, idx) => (
            <FloatingResourceCard key={resource.title} resource={resource} idx={idx} />
          ))}
        </div>
      </section>
